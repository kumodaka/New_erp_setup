...
class Order:
    ...
    def get_all_active_orders_summary(self):
        """Fetches only orders that have not been soft-deleted."""
        sql = """
    ...
    def get_order_details_with_items(self, order_id):
        """Fetches the details for one order and all of its associated items."""
    ...
    def get_deactivated(self):
        sql = """
            SELECT o.id, o.po_number, o.status, c.name as customer_name, o.deactivated_at, o.created_at
            FROM orders o
            LEFT JOIN customers c ON o.customer_id = c.id
            WHERE o.deactivated_at IS NOT NULL OR o.status = 'cancelled'
            ORDER BY o.deactivated_at DESC, o.created_at DESC
        """
        return self.db_helper.execute_statement(sql)
    
    def delete(self, order_id):
        sql = "UPDATE orders SET deactivated_at = %s WHERE id = %s"
        self.db_helper.execute_command(sql, (datetime.now(), order_id))
        
    def get_cancelled_or_rejected_items(self):
        """
        Fetches individual order items that are cancelled or rejected,
        but whose parent order is still active.
        """
        sql = """
            SELECT oi.wo_number, oi.part_number, oi.status, o.po_number, c.name as customer_name
            FROM order_items oi
            JOIN orders o ON oi.order_id = o.id
            JOIN customers c ON o.customer_id = c.id
            WHERE oi.status IN ('cancelled', 'rejected')
            AND o.deactivated_at IS NULL -- Only from active orders
